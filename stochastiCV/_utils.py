...
def _model_repeat(X_, y_, X_test_, y_test_, threshold, model, model_repeats, num_classes, avg_strategy, j, verbose, class_labels):
    df = pd.DataFrame()
...
def _machine_report(X_, y_, X_test_, y_test_, threshold, model, num_classes, avg_strategy, j, verbose, class_labels):
    '''
    Used for comparative purposes, within Subsampling and KFolds machines.
    '''
    df = pd.DataFrame()
    ...
    report = pd.DataFrame({'j':j}, index=[0]) 
    ...
    report.set_index(['j'], inplace=True)
    df = df.append(report)
